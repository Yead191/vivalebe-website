...
export function ChatMessages({
  ...
  const [unlockingMessageId, setUnlockingMessageId] = useState<string | null>(
    null,
  );
  ...
  const otherParticipant = isGroup
    ? null
    : Array.isArray(activeUser?.participants)
    ...
    <div className="flex flex-col h-full bg-[#F8FAFC] relative overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 bg-white/90 backdrop-blur-md z-10 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <Avatar
            src={
              typeof headerImage === "string" && headerImage.startsWith("http")
                ? headerImage
                : (typeof headerImage === "string"
                    ? getImageUrl(headerImage)
                    : undefined) || "/user.png"
            }
            online={!isGroup}
          />
      ...
      >
        ...
        {messages?.map((msg, idx) => {
          ...
          if (isGroup && !isMe) {
            senderParticipant = activeUser?.participants?.find(
              (p: any) => p._id === msg.sender,
            );
            ...
            >
                ...
                <div className="w-8 h-8 shrink-0">
                  {showAvatar ? (
                    <Image
                      src={
                        typeof (
                          senderParticipant?.profile || senderParticipant?.image
                        ) === "string"
                          ? getImageUrl(
                              senderParticipant?.profile ||
                                senderParticipant?.image,
                            ) || "/user.png"
                          : "/user.png"
                      }
                      alt=""
              ...
              >
                {!isMe && isGroup && showAvatar && senderParticipant && (
                  <p className="text-[11px] font-semibold text-gray-500 ml-1">
                ...
                >
                  {/* Image(s) from message */}
                  {(() => {
                    const imgs = Array.isArray(msg.image)
                      ? msg.image.filter(Boolean)
                      : msg.image
                        ? [msg.image]
                        : [];
                    return imgs.length > 0
                      ? imgs.map((imgSrc: string, i: number) => (
                          <div
                            key={i}
                            className="mb-2 rounded-lg overflow-hidden border border-gray-100 relative group/img"
                          >
                            <Image
                              src={
          ...
          <div className="h-full flex flex-col items-center justify-center gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm">
                Send a message to start chatting with {headerName}
              </p>
            </div>
          </div>
        )}
