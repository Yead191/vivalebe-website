...
export default function DiseaseQnAFeature({
  lang,
  dict,
}: {
  lang: Locale;
      ...
      <div className="container py-6 sm:py-8">
        ...
        <section className="w-full flex flex-col gap-8 ">
          ...
          <div className="rounded-[1.5rem] border border-[#429CA8]/12 bg-[#429CA8]/6 p-4 lg:hidden">
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[#2b7e87]">
              {dict.diseaseQa.categoryTitle}
            </p>
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => setFilter("All")}
                className={`rounded-full px-3 py-1.5 text-xs ${filter === "All" ? "bg-[#429CA8] text-white" : "bg-white text-slate-700"}`}
              >
              ...
              {diseaseOptions.map((option) => (
                <button
                  ...
                  className={`rounded-full px-3 py-1.5 text-xs ${filter === option ? "bg-[#429CA8] text-white" : "bg-white text-slate-700"}`}
                >
                  {option}
                </button>
              ))}
          ...
          <div className="space-y-5">
            <div className="grid gap-3 rounded-[2rem] border border-white/70 bg-white/95 p-4 shadow-[0_20px_60px_rgba(15,23,42,0.06)]">
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder={dict.diseaseQa.searchPlaceholder}
            ...
            {visiblePosts?.map((post) => (
              <PostHeader
                ...
                onCommentSubmit={(text, anonymous) =>
                  handleCommentSubmit(post.id, text, anonymous)
                }
