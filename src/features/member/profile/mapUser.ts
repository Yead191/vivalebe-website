...
function mapApiUser(u: Record<string, unknown> | null): User | null {
...
export function userFromProfilePayload(data: unknown): User | null {
  if (!data || typeof data !== "object") return null;
  const record = data as Record<string, unknown>;
  const nested = (
    Array.isArray(record)
      ? record[0]
      : record.user || record.toUser || record.matchedUser || record
  ) as Record<string, unknown> | null;
  return mapApiUser(nested);
}
